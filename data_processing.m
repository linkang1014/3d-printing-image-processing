...
discription1 = sprintf('part1_SI303920180117141814_0%d%d_%s_00.mpm_eval_onoffaxis.tif',k_1,k_2,k_3);
...
x = 1:13;
for n = 1:13
proportion(k/20-109,n) = yellow(k/20-109,n)/(black(k/20-109,n)+yellow(k/20-109,n));
end
plot(x,proportion(k/20-109,:),'r.','markersize',30);
hold on;
grid on;
set(gca,'xtick',x);
xlabel(gca,'position of bar','FontSize',16);
ylabel(gca,'proportion of yellow','FontSize',16);
set(gca,'FontSize',14)
discription2 = sprintf('Proportion of yellow in each bar  Layer %dmm',k);
